...
Highlight geometry now follows actual PDF viewport transforms, crop boxes, rotation, font metrics and proportional boundary-run widths. Eight real-PDF crop/rotation cases went from **1/8 to 8/8** passing against rasterized glyph bounds. Highlights no longer bridge a blank column gutter. For Helvetica `iiiiWWWW`, the `iiii` width error fell from **28.88 PDF points to less than 1 point**. All cases pass in Chromium, Firefox and WebKit. Original extraction offsets and whitespace remain intact.
