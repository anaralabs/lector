...
export const useSelectionDimensions = () => {
        ...
        lineGroups.forEach(group => {
          ...
          while (i < group.length) {
            ...
            while (endIndex + 1 < group.length) {
              ...
              const maxGapAllowed = Math.max(MERGE_THRESHOLD, currentRect.height * 0.3); // Allow gap up to 30% of height
  ...
  const consolidateUnderlines = (underlines: HighlightRect[]): HighlightRect[] => {
    ...
    for (let i = 1; i < sorted.length; i++) {
      ...
      if (horizontallyConnected) {
        ...
        const newWidth = Math.max(current.left + current.width, next.left + next.width) - 
