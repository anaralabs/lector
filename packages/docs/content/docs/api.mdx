...
| `loader` | Accessible loading status | Content shown until all page metadata is ready, or the requested page is ready in progressive mode |
| `errorFallback` | Accessible error message | Receives `{ error, phase, source }` to customize initialization errors; replaces the loader |
| `onDocumentProgress` | — | Receives `{ loaded, total, source }` for download bytes; does not imply painted page readiness |
| `onDocumentLoad` | — | Receives `{ proxy, source }` after PDF.js loads the document, before viewport generation |
| `onError` | — | Receives `{ error, phase, source }` for initialization failures and background page failures in progressive mode |
| `documentOptions` | Lector defaults | PDF.js loading-parameter overrides; applied on the next load |
| `progressive` | `false` | Show the requested page before acquiring remaining pages; unknown dimensions are estimated |
| `initialPage` | `1` | One-based page to acquire/display first; clamped to the page count |
...
`source` changes trigger loading. Keep object and binary sources stable. The zoom props initialize state; use store actions for subsequent changes. Changing `documentOptions`, `progressive`, or `initialPage` alone does not reload the document. See [loading documents](/docs/document-loading) for source handling, retries, and asset paths.
...
| `Pages.copyOptions` | Preserve line breaks | Plain-text copy settings `{ lineBreaks?: "preserve" \| "space", ignoreHyphenation?: boolean }`; `false` keeps native copying |
...
| `pageProxies` | Complete ordered array in eager mode; empty until complete in progressive mode |
| `pagesLoaded` | Whether the complete page array is available |
| `getPdfPageProxy(pageNumber)` | Synchronous one-based accessor; throws if the page is unresolved |
| `loadPdfPageProxy(pageNumber)` | Resolves a real PDF.js page proxy; prioritizes an unresolved progressive page |
| `loadPdfPageProxies()` | Resolves the complete ordered page array |
| `isPageLoaded(pageNumber)` | Tests page availability |
...
| `usePdfSelection()` | Under `Root` with `Pages`; reactive selection status, persistent page/text anchors, sync/async complete text and geometry, restoration and clearing; see [text selection](/docs/code/select) |
| `useSelectionDimensions()` | Under `Root`; `getDimension()` reads text and rectangles, `getAnnotationDimension()` also computes underlines, and `getText(options?)` reads a normalized quote without geometry |
