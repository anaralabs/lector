...
`initialPage` is one-based, defaults to 1, and is clamped to the document's page count. The requested page is acquired first. Remaining pages use at most 16 concurrent requests; newly visible pages move ahead of queued background work. `initialPage` and `progressive` are read when `source` changes. `Pages.initialOffset`, when provided, takes precedence over the initial page's position.
