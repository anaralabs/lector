...
The first Windows run exposed a platform assumption in a selection test: native double-click word selection can include the following space. The test now verifies the captured native endpoint, exactly one character of Shift+Arrow extension, and an unchanged anchor. Runtime selection behavior was not changed. Windows Firefox additionally exposed a real 1.33 CSS-pixel partial-highlight mismatch: PDF.js substitutes Calibri for its selectable sans-serif text, while the highlight code measured the generic family. Partial-highlight measurement now reads PDF.js’s own `TextLayer.fontFamilyMap`. A local forced-font-map reproduction failed by 28.87 pixels before the correction; it now passes the unchanged one-pixel limit. Windows browser tests bind to an IPv4 loopback port outside the default high-port range after a runner rejected Vitest’s default port with `EACCES`. Browser dependencies are explicitly pre-optimized after Vite dependency discovery reloaded a test mid-run and caused invalid React hook calls.

Local validation: 37 Node tests, 264 browser tests in each of Chromium, Firefox and WebKit, types, library build, size limit and packed exports/ESM/SSR checks pass. Bundle size changes from 58,292 to 58,470 bytes (+178 bytes). The new regressions failed on the baseline for unnecessary exact-search indexing and missed page-boundary yields; they also cover timer fallback cancellation and ligature source spans.
